...
final case class Printer(
  ...
  /**
    ...
    */
  final def pretty(j: JsonValue): String = {
    ...
    def appendJsonString(
      ...
      if (normalChars) {
        ...
        while (i < jsonString.length && Printer.isNormalChar(
                 jsonString.charAt(i))) {
          i += 1
        }
    ...
    def trav(depth: Int, k: JsonValue): Unit = {
      val p = pieces(depth)
      ...
      k match {
        case JsonObject(o) =>
          ...
          while (itemIterator.hasNext) {
            ...
            if (!dropNullKeys || value != JsonNull) {
              if (!first) {
        ...
        case JsonString(s)  => encloseJsonString(s)
        case JsonNumber(n)  => builder.append(n.toString)
        case JsonBoolean(b) => builder.append(if (b) trueText else falseText)
        case JsonArray(arr) =>
          val arrIterator = arr.iterator
          if (!arrIterator.hasNext) builder.append(p.lrEmptyBrackets)
          else {
        ...
        case JsonNull => builder.append(nullText)
      }
    }
...
final object Printer {
  ...
  private[json] final def escape(c: Char): String = (c: @switch) match {
    case '\\' => "\\\\"
    case '"'  => "\\\""
    case '\b' => "\\b"
    case '\f' => "\\f"
    ...
    case possibleUnicode =>
      if (Character.isISOControl(possibleUnicode)) {
        "\\u%04x".format(possibleUnicode.toInt)
      } else possibleUnicode.toString
  }

  private[json] final def isNormalChar(c: Char): Boolean = (c: @switch) match {
    case '\\'            => false
    case '"'             => false
    case '\b'            => false
    case '\f'            => false
  ...
  private[json] final case class Pieces(
      lBraces: String,
      rBraces: String,
      lBrackets: String,
      rBrackets: String,
  ...
  private[json] abstract class MemoizedPieces extends Serializable {
    def compute(i: Int): Pieces

    private[this] final val known =
      new java.util.concurrent.CopyOnWriteArrayList[Pieces](
        new Array[Pieces](maxMemoizationDepth)
      )

    final def apply(i: Int): Pieces =
      if (i >= maxMemoizationDepth) compute(i)
      else {
        val res = known.get(i)
